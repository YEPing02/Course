...
public class InClEtapeRallye extends JFrame {
	 ...
	 * @param ed 
	 */
	public InClEtapeRallye(EditionRallye ed) {
		...
		listCl.setBounds(66, 75, 253, 280);
		...
		comboBoxEtape.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					if(comboBoxEtape.getSelectedIndex()!=-1) {	
						EtapeRallye etr=(EtapeRallye)ed.getEtapeT(comboBoxEtape.getSelectedIndex());
						afficherClassement(etr);				
					}
				
				
			}
		});
		comboBoxEtape.setBounds(110, 44, 209, 21);
	...
	private void init(EditionRallye e) {
		comboBoxEtape.removeAllItems();
	       for(int i = 0 ; i<e.getListeEtapeT().size(); i++) {
	    	   EtapeRallye etv= (EtapeRallye)e.getListeEtapeT().get(i);	    	   
	    	   comboBoxEtape.addItem(String.valueOf(etv.getNumEtape()));
	       }
	}
	private void afficherClassement(EtapeRallye et) {
		DefaultListModel<String>  dlm= new DefaultListModel<String>();
		for(Entry<Inscription, Float> e : et.classementEtape()) {
